...
当进入事件循环时，它有一个空队列（fs.readFile()尚未完成），因此定时器将等待剩余毫秒数，当到达 95ms（假设 fs.readFile()需要 95ms）时，fs.readFile()完成读取文件并且其完成需要 10 毫秒的回调被添加到轮询队列并执行。
...
因为 I/O 操作是在 poll 阶段进行的，poll 阶段之后是 check（也就是调用 setImmediate）。
</font>
