...
这种布局乍一看可能很奇怪，但它与 `Node` 的模块解析算法完全兼容！ 解析模块时，`Node` 会忽略符号链接，因此当 f`oo@1.0.0/node_modules/foo/index.js` 需要 `bar` 时，`Node` 不会使用在 `foo@1.0.0/node_modules/bar` 的 `bar`，相反，`bar` 是被解析到其实际位置`（bar@1.0.0/node_modules/bar）`。 因此，`bar` 也可以解析其在 `bar@1.0.0/node_modules` 中的依赖项。
