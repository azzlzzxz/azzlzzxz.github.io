...
1. 重排/回流（Reflow）：当 DOM 的变化影响了元素的几何信息，浏览器需要重新计算元素的几何属性，将其安放在界面中的正确位置，这个过程叫做重排。表现为重新生成布局，重新排列元素。
2. 重绘(Repaint): 当一个元素的外观发生改变，但没有改变布局,重新把元素外观绘制出来的过程，叫做重绘。表现为某些元素的外观被改变。
...
1. 添加、删除、更新 DOM 节点-触发重排和重绘
2. 通过 display: none 隐藏一个 DOM 节点-触发重排和重绘
3. 通过 visibility: hidden 隐藏一个 DOM 节点-只触发重绘，因为没有几何变化
4. 移动或者给页面中的 DOM 节点添加动画-触发重排和重绘
5. 添加一个样式表，调整样式属性-触发重排和重绘
6. 用户行为，例如调整窗口大小，改变字号，或者滚动-触发重排和重绘
